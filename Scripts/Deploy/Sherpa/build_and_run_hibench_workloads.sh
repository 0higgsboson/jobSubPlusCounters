...
echo "--------------------------------------------------------------------"
#${CWD}/sherpa_installer.sh package 2.7.1
...
echo "--------------------------------------------------------------------"
pdcp -w ${CLIENT_HOSTNAME}  ${common_src_dir}/TzCtCommon/target/TzCtCommon-1.0-jar-with-dependencies.jar   ${client_install_dir}/

if [[ ${HADOOP_VERSION} == *"2.7"* ]]
then
    pdcp -w ${CLIENT_HOSTNAME}  ${mr_client_src_dir}/${MR_SRC_DIR}/target/hadoop-mapreduce-client-core-2.7.1.jar   ${client_install_dir}/
else
    pdcp -w ${CLIENT_HOSTNAME}  ${mr_client_src_dir}/${MR_SRC_DIR}/target/hadoop-mapreduce-client-core-2.6.0.jar   ${client_install_dir}/
fi

pdcp -w ${CLIENT_HOSTNAME}  ${hive_client_src_dir}/hiveClientSherpa/cli/target/hive-cli-1.2.1.jar   ${client_install_dir}/
pdcp -w ${CLIENT_HOSTNAME}  ${hive_client_src_dir}/hiveClientSherpa/ql/target/hive-exec-1.2.1.jar   ${client_install_dir}/
...
pdcp -w ${TENZING_HOSTNAME}  ${tenzing_src_dir}/Tenzing/RestServices/target/tenzing-services.war   ${tenzing_tomcat_dir}/apache-tomcat-${TOMCAT_VERSION}/webapps/
...
pdcp -w ${CA_HOSTNAME}  ${clientagent_src_dir}/ClientAgent/ca-services/target/ca-services.war   ${ca_tomcat_dir}/apache-tomcat-${TOMCAT_VERSION}/webapps/
