...
function printUsage(){
    ...
    echo "./sherpa_build.sh distro_name hadoop_version [build_code]      Packages CA & Client installers for customers "
...
if [[ ${DISTRO_NAME} == "apache" ]]; then
     ...
     elif [[ ${HADOOP_VERSION} == "2.4.2" ]]; then
         MR_REPO_URL=https://github.com/hortonworks/hadoop-release.git
         HIVE_REPO_URL=${git_protocol}performance-sherpa/HDP-hive.git
         MR_SRC_DIR=hadoop-release
         HIVE_SRC_DIR=HDP-hive
